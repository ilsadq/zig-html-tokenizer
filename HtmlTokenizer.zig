...
pub fn next(self: *Self, index: *usize) Token {
    ...
    return state: switch (self.state) {
        ...
        .tag_open => switch (self.buffer[index.*]) {
            '!' => {
                index.* += 1;
        ...
        .end_tag_open => switch (self.buffer[index.*]) {
        ...
        .tag_name => switch (self.buffer[index.*]) {
            '\t', '\x0C', ' ', '\n', '\r' => {
                const end = index.*;
                index.* += 1;
        ...
        .self_closing_start_tag => switch (self.buffer[index.*]) {
            '>' => {
                index.* += 1;
                self.state = .data;
        ...
        .before_attribute_name => switch (self.buffer[index.*]) {
            '\t', '\n', ' ', '\x0C', '\r' => {
                index.* += 1;
        ...
        .after_attribute_name => switch (self.buffer[index.*]) {
            '\t', '\n', ' ', '\x0c', '\r' => {
                index.* += 1;
        ...
        .before_attribute_value => switch (self.buffer[index.*]) {
            '\t', '\n', '\x0c', ' ', '\r' => {
                index.* += 1;
                continue :state .before_attribute_value;
            },
        ...
        .attribute_value_single_quoted => switch (self.buffer[index.*]) {
            '\'' => {
                const end = index.*;
                index.* += 1;
        ...
        .after_attribute_value_quoted => switch (self.buffer[index.*]) {
            '\t', '\n', '\x0c', ' ', '\r' => {
                index.* += 1;
                start = index.*;
                continue :state .before_attribute_name;
        ...
        .attribute_value_double_quoted => switch (self.buffer[index.*]) {
            '"' => {
                const end = index.*;
                index.* += 1;
                self.state = .after_attribute_value_quoted;
        ...
        .attribute_value_unquoted => switch (self.buffer[index.*]) {
            '\t', '\n', '\x0c', ' ', '\r' => {
                index.* += 1;
                continue :state .before_attribute_name;
            },
        ...
        .markup_declaration_open => {
            if (std.mem.startsWith(u8, self.buffer[index.*..], "--")) {
                index.* += 2;
                start = index.*;
                continue :state .comment_start;
            } else if (std.mem.startsWith(u8, self.buffer[index.*..], "DOCTYPE")) {
                index.* += 7;
                start = index.*;
                continue :state .before_doctype_name;
            } else if (std.mem.startsWith(u8, self.buffer[index.*..], "[CDATA[")) {
                index.* += 7;
                continue :state .cdata_section;
            } else continue :state .bogus_comment;
        },
        // https://html.spec.whatwg.org/multipage/parsing.html#bogus-comment-state
        .bogus_comment => switch (self.buffer[index.*]) {
            '>' => {
                const end = index.*;
                index.* += 1;
                self.state = .data;
        ...
        .comment_start => switch (self.buffer[index.*]) {
            '-' => {
                index.* += 1;
        ...
        .comment_start_dash => switch (self.buffer[index.*]) {
            '-' => {
                index.* += 1;
        ...
        .comment_state => switch (self.buffer[index.*]) {
            '-' => {
                index.* += 1;
        ...
        .comment_end_dash => switch (self.buffer[index.*]) {
            '-' => {
                index.* += 1;
                continue :state .comment_end;
            },
        ...
        .comment_end => switch (self.buffer[index.*]) {
            '>' => {
                const end = index.* - 2;
                index.* += 1;
                self.state = .data;
        ...
        .before_doctype_name => switch (self.buffer[index.*]) {
            '\t', '\n', '\x0C', ' ', '\r' => {
                index.* += 1;
                start = index.*;
                continue :state .before_doctype_name;
        ...
        .doctype => switch (self.buffer[index.*]) {
            '>' => {
                const end = index.*;
                index.* += 1;
                self.state = .data;
        ...
        .cdata_section => switch (self.buffer[index.*]) {
            ']' => {
                index.* += 1;
        ...
        .cdata_section_bracket => switch (self.buffer[index.*]) {
            ']' => {
                index.* += 1;
        ...
        .cdata_section_end => switch (self.buffer[index.*]) {
            '>' => {
                const end = index.*;
                index.* += 1;
                self.state = .data;
